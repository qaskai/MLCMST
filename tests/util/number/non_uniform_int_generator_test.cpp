...
TEST_CASE( " Non-uniform Int generator (non-deterministic) ", "[generation][number]" )
{
    using MLCMST::util::number::NonUniformIntGenerator;
    int base = 10;
    int iterations = 1000;
    std::vector<double> probabilities { 0.2, 0.1, 0.3, 0.4 };
    NonUniformIntGenerator generator(base, probabilities);
    ...
    for (int i=0; i < probabilities.size(); i++) {
        ...
        REQUIRE( Approx(percentage).margin(0.05) == probabilities[i] );
    }
}
